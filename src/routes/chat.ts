...
const askRoute = createRoute({
  ...
  description:
    "Chat with Kay AI assistant. Can interact with Jira, Bitbucket, Confluence, and k-mesh services. Use ?interactive=true for streaming conversation.",
  security: [{ bearerAuth: [] }],
  request: {
    query: z.object({
      interactive: z
...
export function createChatRouter(registry: MCPServerRegistry) {
  ...
  router.openapi(askRoute, async (c) => {
    try {
      ...
      if (interactive && result instanceof Response) {
        return result;
      }

      if (!interactive && typeof result === "object" && "response" in result) {
        return c.text(result.response as string, 200, {
